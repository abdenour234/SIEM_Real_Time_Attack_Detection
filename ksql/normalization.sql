...
-----------------------------------------
-- 1) RAW STREAM (contains both formats)
-----------------------------------------
...
-----------------------------------------
-- 2) PRODUCER 1 NORMALIZER
-----------------------------------------
CREATE STREAM producer1_events AS
SELECT
...
-----------------------------------------
-- 3) PRODUCER 2 NORMALIZER
-----------------------------------------
CREATE STREAM producer2_events AS
SELECT
    evt_id AS event_id,
...
-----------------------------------------
-- 4) FINAL NORMALIZED AVRO STREAM FROM PRODUCER 1
-----------------------------------------
CREATE STREAM normalized_events_stream
WITH (
...
SELECT
    ...
    longitude,
    request_latency_ms,
    COALESCE(auth_method, 'unknown') AS auth_method,
    account_age_days,
    uses_vpn,
    device_fingerprint,
    source_producer,
...
-----------------------------------------
-- 5) INSERT PRODUCER 2 EVENTS INTO THE SAME STREAM
-----------------------------------------
